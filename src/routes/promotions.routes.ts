...
function crud(path: string, table: string, select: string, schema: any, columns: Record<string, string>, needsImage = false, formatter: (req: any, row: any) => any = (_req, row) => row, parseFormData = false) {
  const bodyParser = needsImage ? [uploadPromotionImage.single("image")] : parseFormData ? [uploadPromotionImage.none()] : [];
