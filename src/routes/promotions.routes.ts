...
const flashInput = z.object({ name: z.string().trim().min(2).max(150), minimumDiscount: money, startAt: dateTime, endAt: dateTime, description: z.string().trim().min(2).max(10_000), image: imageFilename.nullable().optional(), isActive: boolean.optional() }).refine(value => new Date(value.endAt) > new Date(value.startAt), { message: "endAt must be after startAt", path: ["endAt"] });
const bannerInput = z.object({ title: z.preprocess(value => value === "" ? null : value, z.string().trim().max(255).nullable().optional()), image: imageFilename, isOwnShop: boolean.optional(), isActive: boolean.optional() });
const adInput = z.object({ title: z.string().trim().min(2).max(255), image: imageFilename, isActive: boolean.optional() });
const promoInput = z.object({ shopIds, code: z.string().trim().min(2).max(100).transform(value => value.toUpperCase()), discountType: z.enum(["amount", "percent"]), discount: money, minimumOrderAmount: money, singleUserLimit: z.coerce.number().int().positive().default(1), maximumDiscountAmount: z.preprocess(value => value === "" ? null : value, money.nullable().optional()), startAt: dateTime, expireAt: dateTime, isActive: boolean.optional() }).refine(value => new Date(value.expireAt) > new Date(value.startAt), { message: "expireAt must be after startAt", path: ["expireAt"] });
...
crud("/flash-sales", "flash_sales", "SELECT id, name, minimum_discount AS minimumDiscount, starts_at AS startAt, ends_at AS endAt, description, image, is_active AS isActive, created_at AS createdAt, updated_at AS updatedAt", flashInput, { name: "name", minimumDiscount: "minimum_discount", startAt: "starts_at", endAt: "ends_at", description: "description", image: "image", isActive: "is_active" }, true, image);
...
crud("/promo-codes", "promo_codes", "SELECT id, shop_ids AS shopIds, code, discount_type AS discountType, discount, minimum_order_amount AS minimumOrderAmount, single_user_limit AS singleUserLimit, maximum_discount_amount AS maximumDiscountAmount, starts_at AS startAt, expires_at AS expireAt, is_active AS isActive, created_at AS createdAt, updated_at AS updatedAt", promoInput, { shopIds: "shop_ids", code: "code", discountType: "discount_type", discount: "discount", minimumOrderAmount: "minimum_order_amount", singleUserLimit: "single_user_limit", maximumDiscountAmount: "maximum_discount_amount", startAt: "starts_at", expireAt: "expires_at", isActive: "is_active" }, false, (_req, row) => ({ ...row, shopIds: typeof row.shopIds === "string" ? JSON.parse(row.shopIds) : row.shopIds }));
