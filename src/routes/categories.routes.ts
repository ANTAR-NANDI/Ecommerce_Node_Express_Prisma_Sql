...
categoriesRouter.post("/", requireAuth, requireAdmin, categoryUpload, asyncHandler(async (req, res) => {
  // Multipart requests can include an image and banner; JSON can send uploaded filenames.
  const input = categoryInput.parse({ ...req.body, ...uploadedCategoryImages(req) });
...
categoriesRouter.patch("/:id", requireAuth, requireAdmin, categoryUpload, asyncHandler(async (req, res) => {
  const input = categoryInput.partial().parse({ ...req.body, ...uploadedCategoryImages(req) }); if (!Object.keys(input).length) throw new HttpError(400, "Provide at least one field to update");
