...
accountsRouter.get("/banks/:id", asyncHandler(async (req, res) => { const bankId = id.parse(req.params.id); const [rows] = await db.execute<any[]>(`${bankSelect} WHERE id = ?`, [bankId]); if (!rows[0]) throw new HttpError(404, "Bank not found"); res.json({ success: true, data: rows[0] }); }));
