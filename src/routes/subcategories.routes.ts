...
const inputSchema = z.object({ categoryId: z.coerce.number().int().positive(), name: z.string().trim().min(2).max(100), slug: z.string().trim().min(2).max(120).regex(/^[a-z0-9-]+$/), imageUrl: z.string().url().max(500).nullable().optional(), isActive: boolean.optional() });
const select = "SELECT s.id, s.category_id AS categoryId, s.name, s.slug, s.image_url AS imageUrl, s.is_active AS isActive, s.created_at AS createdAt, s.updated_at AS updatedAt, c.name AS categoryName FROM subcategories s JOIN categories c ON c.id = s.category_id";
...
subcategoriesRouter.post("/", requireAuth, requireAdmin, asyncHandler(async (req, res) => {
  const input = inputSchema.parse(req.body);
  const [result] = await db.execute<any>("INSERT INTO subcategories (category_id, name, slug, image_url, is_active) VALUES (?, ?, ?, ?, ?)", [input.categoryId, input.name, input.slug, input.imageUrl ?? null, input.isActive ?? true]);
...
subcategoriesRouter.patch("/:id", requireAuth, requireAdmin, asyncHandler(async (req, res) => {
  ...
  const names: Record<string, string> = { categoryId: "category_id", imageUrl: "image_url", isActive: "is_active", name: "name", slug: "slug" };
