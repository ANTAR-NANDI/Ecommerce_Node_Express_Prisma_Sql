...
const input = z.object({ title: z.string().trim().min(2).max(255), slug: z.string().trim().min(2).max(280).regex(/^[a-z0-9-]+$/), category_id: id, tags, description: z.string().trim().min(2).max(50_000), image: imageFilename.nullable().optional(), isActive: boolean.optional() });
const select = "SELECT b.id, b.title, b.slug, b.category_id, c.name AS categoryName, b.tags, b.description, b.image, b.is_active AS isActive, b.created_at AS createdAt, b.updated_at AS updatedAt FROM blogs b JOIN categories c ON c.id = b.category_id";
...
blogsRouter.get("/", asyncHandler(async (req, res) => {
  ...
  // Some MySQL deployments reject prepared-statement placeholders in LIMIT/OFFSET.
  // These pagination values are validated integers, while filter values remain parameterized.
  const [rows] = await db.query<any[]>(`${select}${where} ORDER BY b.created_at DESC LIMIT ${query.limit} OFFSET ${offset}`, values);
...
blogsRouter.post("/", requireAuth, requireAdmin, uploadBlogImage.single("image"), asyncHandler(async (req, res) => { const body = input.parse({ ...req.body, image: req.file?.filename ?? req.body?.image }); await ensureCategoryExists(body.category_id); const [result] = await db.execute<any>("INSERT INTO blogs (title, slug, category_id, tags, description, image, is_active) VALUES (?, ?, ?, ?, ?, ?, ?)", [body.title, body.slug, body.category_id, JSON.stringify(body.tags), body.description, body.image ?? null, body.isActive ?? true]); const [rows] = await db.execute<any[]>(`${select} WHERE b.id = ?`, [result.insertId]); res.status(201).json({ success: true, data: format(req, rows[0]) }); }));
blogsRouter.patch("/:id", requireAuth, requireAdmin, uploadBlogImage.single("image"), asyncHandler(async (req, res) => { const blogId = id.parse(req.params.id); const updateBody = { ...(req.body ?? {}) }; if (req.file) updateBody.image = req.file.filename; const body = input.partial().parse(updateBody); if (!Object.keys(body).length) throw new HttpError(400, "Provide at least one field to update"); if (body.category_id) await ensureCategoryExists(body.category_id); const columns: Record<string, string> = { title: "title", slug: "slug", category_id: "category_id", tags: "tags", description: "description", image: "image", isActive: "is_active" }; const fields = Object.entries(body).map(([key, value]) => ({ column: columns[key]!, value: key === "tags" ? JSON.stringify(value) : value })); const [result] = await db.query<any>(`UPDATE blogs SET ${fields.map(field => `${field.column} = ?`).join(", ")} WHERE id = ?`, [...fields.map(field => field.value), blogId] as any); if (!result.affectedRows) throw new HttpError(404, "Blog not found"); const [rows] = await db.execute<any[]>(`${select} WHERE b.id = ?`, [blogId]); res.json({ success: true, data: format(req, rows[0]) }); }));
