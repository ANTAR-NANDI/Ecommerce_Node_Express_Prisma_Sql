...
const input = z.object({ title: z.string().trim().min(2).max(255), slug: z.string().trim().min(2).max(280).regex(/^[a-z0-9-]+$/), categoryId: id, tags, description: z.string().trim().min(2).max(50_000), image: imageFilename.nullable().optional(), isActive: boolean.optional() });
const select = "SELECT b.id, b.title, b.slug, b.category_id AS categoryId, c.name AS categoryName, b.tags, b.description, b.image, b.is_active AS isActive, b.created_at AS createdAt, b.updated_at AS updatedAt FROM blogs b JOIN categories c ON c.id = b.category_id";
...
blogsRouter.get("/", asyncHandler(async (req, res) => {
  const admin = req.baseUrl.startsWith("/admin/"); const query = z.object({ categoryId: id.optional(), page: z.coerce.number().int().positive().default(1), limit: z.coerce.number().int().min(1).max(100).default(12) }).parse(req.query);
  const filters: string[] = []; const values: number[] = []; if (!admin) filters.push("b.is_active = TRUE"); if (query.categoryId) { filters.push("b.category_id = ?"); values.push(query.categoryId); }
