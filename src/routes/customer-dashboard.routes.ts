...
const addressInput = z.object({ label: z.string().trim().min(1).max(100), recipientName: z.string().trim().min(2).max(150), phone: z.string().trim().min(6).max(30), address: z.string().trim().min(5).max(2000), city: optionalText(100), area: optionalText(100), postalCode: optionalText(30), isDefault: z.preprocess(value => value === "true" ? true : value === "false" ? false : value, z.boolean().optional()) });
const addressSelect = "SELECT id, label, recipient_name AS recipientName, phone, address, city, area, postal_code AS postalCode, is_default AS isDefault, created_at AS createdAt, updated_at AS updatedAt FROM customer_addresses";
...
const dashboard = asyncHandler(async (req, res) => {
  ...
  const [[orders], [wishlist], [addresses], [messages]] = await Promise.all([
    ...
    db.execute<any[]>("SELECT id, label, recipient_name AS recipientName, phone, address, city, area, postal_code AS postalCode FROM customer_addresses WHERE customer_id = ? AND is_default = TRUE LIMIT 1", [currentCustomerId]),
...
customerDashboardRouter.get("/wishlist", asyncHandler(async (req, res) => {
  const [rows] = await db.execute<any[]>("SELECT pf.product_id AS productId, pf.created_at AS createdAt, p.name, p.slug, p.selling_price AS sellingPrice, p.image_url AS image FROM product_favorites pf JOIN products p ON p.id = pf.product_id WHERE pf.customer_id = ? ORDER BY pf.created_at DESC", [customerId(req)]);
  res.json({ success: true, data: rows });
}));
